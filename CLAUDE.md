...
**Hometown matching is exact, not fuzzy, by design.** `findCity()` requires an exact "City, ST" match (case-insensitive) against `usCities.js`, fed by an HTML `<datalist>` for autocomplete. This was a deliberate choice over fuzzy/partial matching: several city names repeat across states in the list (Springfield in IL/MA, Portland in ME/OR, Charleston in WV/SC) — guessing which one a bare "Springfield" meant would silently anchor the whole distance calculation to a potentially wrong state. An unmatched input shows an inline "we don't recognize that" message rather than falling back to a guess. A city not in the curated list (small towns aren't covered) has no fallback yet — the user's only recourse today is typing the nearest big city themselves; consider whether that's worth surfacing more explicitly if it comes up.

Originally this used a US-state dropdown (`usStateCoords.js`, now deleted) instead of a hometown — replaced because state-level granularity was too coarse (e.g. Newark vs. Cape May, NJ are ~150 miles apart but both just "NJ"). A ZIP-code-based version would be more precise still, but needs a real ZIP-centroid dataset (tens of thousands of rows) rather than something hand-curatable — not pursued for now.
